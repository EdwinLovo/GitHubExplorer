...
| Annotation definitions | `GhePreview.kt` — the `@GhePreview` and `@GhePreviewScreen` annotations + `GitHubExplorerPreviewContainer` | Compose preview annotations |
| Composable helpers | `ObserveAsEvents.kt`, `OnLifecycleResumed.kt` | Small `@Composable` helpers that don't belong in `components/` |
| Icons registry | `GheIcons.kt` | Central mapping of `GheIcons.X = R.drawable.ic_x` (see `rules/icons.md`) |
