...
- **`MainDispatcherRule`** replaces `Dispatchers.Main` with `UnconfinedTestDispatcher` — put it in `app/src/test/java/edwinlovo.githubexplorer/testutil/MainDispatcherRule.kt`
...
Fakes live next to the interface they fake, under `app/src/test/java/edwinlovo.githubexplorer/data/repository/fakes/`.
