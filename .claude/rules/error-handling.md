...
Copy these three files verbatim into any new project (`edwinlovo.githubexplorer/presentation/utils/`). The rest of this rule assumes they exist.
...
3. **`MainScreen`** observes the bus once at the top of the compose tree:
    ...
    ObserveAsEvents(ErrorEventBus.events) { uiText ->
        // set local state that renders GitHubExplorerErrorAlertDialog
    }
    ```
