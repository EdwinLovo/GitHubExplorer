...
Copy these two files into `edwinlovo.githubexplorer/presentation/utils/`. `OnLifecycleResumed` is the required baseline; add `OnLifecycleEvent` the first time a screen needs a non-resume event.
