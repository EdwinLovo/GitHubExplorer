...
- `BaseRepository`, `ApiResult`, or `safeApolloCall*` / `safeCallSuspend` referenced but the files don't exist in `edwinlovo.githubexplorer/data/repository/` and `edwinlovo.githubexplorer/domain/utils/` → invoke the `repository-primitives` skill
