...
Source every design value from `GheTheme`. `MaterialTheme` is banned in UI code with a single exception.
...
| Need | Use | Never |
|---|---|---|
| Colors | `GheTheme.colors.<token>` | `MaterialTheme.colorScheme.*`, `Color(...)`, `Color.Red` |
| Spacing (between siblings) | `GheTheme.spacing.<size>` | Inline `.dp` |
| Padding (inside a container) | `GheTheme.padding.<size>` | Inline `.dp` |
| Corner radius | `GheTheme.cornerRadius.<size>` | `RoundedCornerShape(8.dp)`, `MaterialTheme.shapes.*` |
| Icon size | `GheTheme.iconSize.<size>` | Inline `.dp` |
| Border width | `GheTheme.borderWidth.<size>` | Inline `.dp` |
...
If a `.dp`, `Int`, or `Float` is genuinely one-off (used exactly once, nothing similar exists in `GheTheme`), declare it as a private file-level constant at the top of the file:
...
2. Populate light and dark values in `GheTheme` initializers
...
3. **Material** (`colors/{Light,Dark}MaterialColorScheme.kt`, `internal`) — thin mapping from primitives to Material3 roles (`primary`, `onPrimary`, `background`, `error`, `outline`). Consumed only by `MaterialTheme(colorScheme = ...)` inside the `GheTheme` composable; UI code never reads from here.
...
// ✅ Correct
Text("Hi", color = GheTheme.colors.primary)
Spacer(modifier = Modifier.height(GheTheme.spacing.large))
Card(shape = RoundedCornerShape(GheTheme.cornerRadius.medium)) { }
```
