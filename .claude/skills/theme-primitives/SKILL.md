...
description: First-time setup for the theme layer — reference implementations for the theme composable, accessor object, primitive/extended color layers, Material color schemes, dimension tokens, and typography. Invoke when scaffolding theming in a new project, when a rule references GheTheme.colors / GheTheme.spacing / etc. and the files don't exist, or when adding a new color token or dimension size.
allowed-tools:
  - Read
  - Write
  - Edit
...
Reference implementations for the seven files that back the theming pattern documented in `.claude/rules/theming-and-tokens.md`. Copy them into `edwinlovo.githubexplorer/presentation/ui/theme/` when setting up a new project. Every `GheTheme.colors.<token>`, `GheTheme.spacing.<size>`, `GheTheme.padding.<size>`, `GheTheme.cornerRadius.<size>`, `GheTheme.iconSize.<size>`, and `GheTheme.borderWidth.<size>` reference in the toolkit assumes they exist.
...
3. **Material** (`colors/{Light,Dark}MaterialColorScheme.kt`, `internal`) — thin mapping from primitives to Material3 roles (`primary`, `onPrimary`, `background`, `error`, `outline`). Consumed only by `MaterialTheme(colorScheme = ...)` inside the `GheTheme` composable.
...
**Naming note** — the `GheTheme` placeholder maps to a single Kotlin identifier used both as the theme composable and as the accessor object (same pattern as Material3's `MaterialTheme { }` composable + `MaterialTheme` object). If your project prefers separate names (e.g. `AppTheme` composable + `AppTokens` accessor), rename after substitution.

---

## File 1 — `Theme.kt`

The theme composable + accessor object. `MainActivity` wraps its content in `GheTheme { AppNavHost(...) }`; every UI file reads tokens through the accessor.

Path: `edwinlovo.githubexplorer/presentation/ui/theme/Theme.kt`
...
The composable and object intentionally share the name — Kotlin resolves `GheTheme { ... }` as the function call and `GheTheme.colors` as the object member. Rename `lightAppColors` / `darkAppColors` to match your project (e.g. `lightAcmeColors`) before or after substitution.

---

## File 2 — `Type.kt`

Typography scaffold. Consumed by `MaterialTheme(typography = Typography, ...)` inside `GheTheme`. UI code reads typography through `MaterialTheme.typography.<style>` — this is the **only** valid `MaterialTheme.*` accessor per `rules/theming-and-tokens.md`.

Path: `edwinlovo.githubexplorer/presentation/ui/theme/Type.kt`
...
Dimension tokens grouped by category. `GheTheme` exposes each nested object through a get-only property (`GheTheme.spacing`, `GheTheme.padding`, etc.), so UI code reads `GheTheme.spacing.md` — plain `object` access, no CompositionLocal.

Path: `edwinlovo.githubexplorer/presentation/ui/theme/Dimens.kt`
...
The raw palette. **`internal` — never referenced from UI code.** UI reads `GheTheme.colors.<semanticName>` via the extended layer. This file is consumed only by `ExtendedColors` implementations (File 5) and the Material color schemes (Files 6–7).

Path: `edwinlovo.githubexplorer/presentation/ui/theme/colors/PrimitiveColors.kt`
...
Path: `edwinlovo.githubexplorer/presentation/ui/theme/colors/ExtendedColors.kt`
...
**Adding a new token** — declare it in the `interface`, populate both `lightAppColors` and `darkAppColors`, then reference it from UI as `GheTheme.colors.<newToken>`. If a semantic token needs a shade that doesn't exist in the primitive palette yet, add the primitive first.

---

## File 6 — `colors/LightMaterialColorScheme.kt`

Thin mapping from primitives to Material3 roles. Consumed only by the `MaterialTheme(colorScheme = ...)` call inside `GheTheme`. UI code doesn't read from here — the extended layer covers everything except when Material3 components (e.g. `Button`, `Card`) need their built-in `colorScheme`-based defaults.

Path: `edwinlovo.githubexplorer/presentation/ui/theme/colors/LightMaterialColorScheme.kt`
...
Path: `edwinlovo.githubexplorer/presentation/ui/theme/colors/DarkMaterialColorScheme.kt`
...
- `presentation/ui/theme/Theme.kt` — `GheTheme` composable + accessor object
...
`MainActivity` wraps its Compose content in `GheTheme { AppNavHost(navController, startDestination) }`. UI code reads tokens through `GheTheme.colors.<x>` / `GheTheme.spacing.<x>` / `GheTheme.padding.<x>` / `GheTheme.cornerRadius.<x>` / `GheTheme.iconSize.<x>` / `GheTheme.borderWidth.<x>`, and typography through `MaterialTheme.typography.<style>`. See `rules/theming-and-tokens.md` for the full usage rules and violation list.
