...
3. **`Include preview infrastructure`** (`header: Previews`) — options: `Yes` (Recommended) / `No`. Adds the `GhePreview` annotations + `GitHubExplorerPreviewContainer` under `presentation/utils/`.
...
1. **`Project display name`** (`header: Project`) — free text. Becomes `GitHubExplorer` (e.g. `Acme`). Suggest based on the current directory name.
2. **`Package root`** (`header: Package`) — free text. Becomes `edwinlovo.githubexplorer` and `edwinlovo.githubexplorer` (e.g. `com.acme.pos`). Must be lowercase, dot-separated, no dashes.
3. **`Theme accessor + composable name`** (`header: Theme`) — options: `GitHubExplorerTheme` (Recommended) / other. Becomes `GheTheme`. Shared by the theme composable AND accessor object (Material3 pattern).
4. **`Ticket prefix pattern`** (`header: Ticket`) — free text. Becomes `GHE-XXX` (e.g. `ACME-XXX`).

### Round 3 — naming (components)

Ask these four questions in a single `AskUserQuestion` call:

1. **`Scaffold composable name`** (`header: Scaffold`) — options: `GitHubExplorerScaffold` (Recommended) / other. Becomes `GheScaffold`.
2. **`Dialog container name`** (`header: Dialog`) — options: `GitHubExplorerDialogContainer` (Recommended) / other. Becomes `GheDialogContainer`.
3. **`Icons registry name`** (`header: Icons`) — options: `GitHubExplorerIcons` (Recommended) / other. Becomes `GheIcons`.
4. **`Preview annotation name`** (`header: Preview`) — options: `GitHubExplorerPreview` (Recommended) / other. Becomes `GhePreview` (and `GhePreviewScreen` for tablet variants).
...
Placeholder substitutions:
  GitHubExplorer    → <resolved>
  edwinlovo.githubexplorer        → <resolved>
  edwinlovo.githubexplorer          → <resolved>            (same as PKG_ROOT)
  GheTheme           → <resolved>
  GheScaffold        → <resolved>
  GheDialogContainer          → <resolved>
  GheIcons           → <resolved>
  GhePreview         → <resolved>
  GHE-XXX   → <resolved>
  Retrofit         → Retrofit | Apollo
  Paging 3          → yes | no
...
Under `edwinlovo.githubexplorer/` (which typically maps to `app/src/main/java/<pkg-path>/`):
...
4. **`misc-primitives`** — writes 9 utility files under `presentation/utils/` and `presentation/utils/ext/`: `ErrorEventBus.kt`, `UiText.kt`, `ObserveAsEvents.kt`, `OnLifecycleResumed.kt`, `OnLifecycleEvent.kt`, `Constants.kt`, `ext/StateFlowExt.kt`, `GhePreview.kt`, `GheIcons.kt`. Pass the resolved `GhePreview` / `GheIcons` / `GheTheme` / `GitHubExplorer` values so its file writes land pre-substituted.
5. **`component-primitives`** — writes 4 shared UI component files under `presentation/ui/components/scaffold/` and `presentation/ui/components/dialogs/`: `GheScaffold.kt`, `GheDialogContainer.kt`, `GitHubExplorerAlertDialog.kt`, `GitHubExplorerErrorAlertDialog.kt`. Always runs — the MainScreen stub renders `GitHubExplorerErrorAlertDialog`. Pass the resolved `GheScaffold` / `GheDialogContainer` / `GheTheme` / `GitHubExplorer` values.

If Round 1 answered "No" to Include ErrorEventBus, tell `misc-primitives` to skip `ErrorEventBus.kt`, `UiText.kt`, and `ObserveAsEvents.kt`. If Round 1 answered "No" to preview infrastructure, tell it to skip `GhePreview.kt`.
...
### `edwinlovo.githubexplorer/GitHubExplorerApplication.kt`

Register this class in `AndroidManifest.xml` under `<application android:name=".GitHubExplorerApplication">`.

```kotlin
package edwinlovo.githubexplorer
...
### `edwinlovo.githubexplorer/presentation/ux/main/MainActivity.kt`
...
### `edwinlovo.githubexplorer/presentation/ux/main/MainScreen.kt`

The app shell — **does not** wrap in `GheScaffold` (see `rules/screens.md`, app-shell exception). Hosts `AppNavHost` + observes `ErrorEventBus`.
...
fun MainScreen(
    ...
    // TODO — render errorToShow via a GheDialogContainer-based error dialog. See rules/error-handling.md.
}
```

### `edwinlovo.githubexplorer/presentation/ux/main/MainViewModel.kt`
...
### `edwinlovo.githubexplorer/presentation/ui/navigation/AppNavHost.kt`
...
Under `edwinlovo.githubexplorer/data/di/`.
...
- [ ] Register GitHubExplorerApplication in AndroidManifest.xml:
      <application android:name=".GitHubExplorerApplication" ...>

- [ ] Fill in edwinlovo.githubexplorer/presentation/ui/theme/colors/PrimitiveColors.kt with your Figma palette
...
- [ ] Add vector drawables under res/drawable/ and register each in GheIcons.kt as you use them
