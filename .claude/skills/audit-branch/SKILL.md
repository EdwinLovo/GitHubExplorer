...
- `MaterialTheme.colorScheme.*` → must be `GheTheme.colors.*`
- `MaterialTheme.shapes.*` → must be `GheTheme.cornerRadius.*`
- Hardcoded `Color(...)` or `Color.Red` → must be `GheTheme.colors.*`
...
- `Icons.*` from Material → must use `GheIcons.*` + `painterResource()`

**Scaffold & dialogs** (`rules/dialogs.md`)
- `Scaffold { }` used directly → must use `GheScaffold`
- Two scaffolds nested → never nest
- Dialog `@Composable` not wrapped in `GheDialogContainer` as the outermost wrapper
...
- `@Composable` function with no `GhePreview` / `GhePreviewScreen` → add a preview at the bottom of the file
...
- Dialog missing `GheDialogContainer`
