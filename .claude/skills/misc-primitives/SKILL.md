...
The shared UI component stubs (`GheScaffold`, `GheDialogContainer`, `GitHubExplorerAlertDialog`, `GitHubExplorerErrorAlertDialog`) live in the `component-primitives` skill.
...
- `GhePreview` annotations + `GitHubExplorerPreviewContainer` → `rules/previews.md`
- `GheIcons` → `rules/icons.md`
...
## File 8 — `presentation/utils/GhePreview.kt`

The two annotations (component + screen) plus `GitHubExplorerPreviewContainer`. Every `@Composable` preview in the app uses these — see `rules/previews.md`.

Adjust `widthDp` / `heightDp` on `@GhePreviewScreen` for your target device (tablet `1280 × 800` shown here; phone e.g. `411 × 892`).
...
## File 9 — `presentation/utils/GheIcons.kt`

Registry stub. Every icon in the app is looked up through this object — never `Icons.*` (Material Icons). See `rules/icons.md`.

Workflow: (1) drop a vector drawable into `res/drawable/` (e.g. `ic_settings.xml`); (2) register it here; (3) reference from UI as `Icon(painterResource(GheIcons.Settings), contentDescription = ...)`.
...
- `presentation/utils/GhePreview.kt`
- `presentation/utils/GheIcons.kt`

Every rule that references `ErrorEventBus.send(...)`, `uiText.asString()`, `OnLifecycleResumed { }`, `GhePreviewScreen`, `GheIcons.<Name>`, `ABSOLUTE_WEIGHT` / `EMPTY_STRING`, or `uiState.reduce { copy(...) }` will now resolve. See the corresponding rules linked at the top of this skill for usage patterns.
