...
- **Folder tree** under `edwinlovo.githubexplorer/` — `domain/`, `data/` (with `di/`, `mappers/`, `paging/`, `remote/datasource/`, `repository/`), `presentation/` (with `ui/components/*`, `ui/navigation/`, `ui/theme/colors/`, `utils/ext/`, `ux/main/`)
...
- **Utility files** — via the `misc-primitives` skill: `ErrorEventBus.kt`, `UiText.kt`, `ObserveAsEvents.kt`, `OnLifecycleResumed.kt`, `OnLifecycleEvent.kt`, `Constants.kt`, `StateFlowExt.kt`, the `GhePreview` annotations + `GitHubExplorerPreviewContainer`, and the `GheIcons.kt` stub
- **Shared UI components** — via the `component-primitives` skill: `GheScaffold.kt`, `GheDialogContainer.kt`, `GitHubExplorerAlertDialog.kt`, `GitHubExplorerErrorAlertDialog.kt`
- **App-shell stubs** — `GitHubExplorerApplication.kt` (with `@HiltAndroidApp`), `MainActivity.kt` (with `@AndroidEntryPoint` and `setContent { GheTheme { ... } }`), `AppNavHost.kt`, `MainScreen.kt`, `MainViewModel.kt`
...
- **`AndroidManifest.xml`** — register the `GitHubExplorerApplication` yourself (checklist reminds you).
